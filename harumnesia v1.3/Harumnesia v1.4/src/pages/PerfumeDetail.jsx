...
// Enhanced similar parfum component for recommendations with optimized animations
const SimilarPerfume = ({ id, image, brand, name, price }) => {
  ...
  return (
    <Link to={`/perfume/${id}`} className="block group">
      <div className="relative bg-gradient-to-br from-white via-white to-cream rounded-2xl overflow-hidden shadow-lg border border-gold/20 hover:border-gold/40 transform-gpu hover-lift-card">
        ...
        <div className="relative z-10 bg-gradient-to-br from-white via-white to-cream rounded-2xl">
          ...
          <div className="relative overflow-hidden rounded-t-2xl">
            <img
              src={imgSrc}
              alt={name}
              className="w-full h-32 sm:h-40 object-cover transform-gpu transition-transform duration-200 group-hover:scale-105"
            />
          ...
          <div className="p-3 sm:p-4">
            <div className="mb-1">
              <span className="text-dark-blue font-inter text-xs uppercase tracking-wider font-semibold smooth-colors group-hover:text-gold">
                {brand}
              </span>
            </div>
            <h2 className="font-playfair font-bold text-base sm:text-lg text-dark-gray mb-2 smooth-colors group-hover:text-dark-blue line-clamp-2">
              {name}
            </h2>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              <span className="bg-gradient-to-r from-black via-dark-gray to-black text-white font-inter font-medium text-xs sm:text-sm px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg shadow-lg shadow-smooth group-hover:shadow-xl text-center">
                {price || "Price unavailable"}
              </span>
              <span className="text-light-gray font-inter text-xs sm:text-sm smooth-colors group-hover:text-gold font-semibold text-center sm:text-right">
                30ml XDP
              </span>
            </div>
          </div>
...
const PerfumeDetail = () => {
  ...
  const fetchSimilarPerfumes = async (brandName) => {
    try {
      ...
      const allPerfumes = response.data || response || [];

      // Ensure data is an array before processing
      if (!Array.isArray(allPerfumes)) {
