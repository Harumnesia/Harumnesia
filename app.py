...
import pickle
from flask_cors import CORS # Library CORS sudah diimpor, bagus!
import numpy as np
import pandas as pd
...
# ✔️ KONFIGURASI CORS DIPERBARUI UNTUK KEAMANAN PRODUKSI
# ====================================================================
# Baris ini mengaktifkan CORS hanya untuk origin (domain) yang spesifik.
# Ini lebih aman daripada mengizinkan semua domain (*).

# Daftar origin yang diizinkan:
# 1. Frontend saat development di lokal (Vite/React)
# 2. Frontend setelah di-deploy ke domain utama
origins = [
    "http://localhost:5173",
    "https://harumnesia.web.id"
]

# Terapkan CORS dengan daftar origin yang sudah ditentukan.
# 'resources' menargetkan endpoint mana yang akan menerapkan aturan ini.
# r"/*" berarti berlaku untuk semua endpoint di aplikasi ini.
...
# --- Inisialisasi untuk WSGI server (aaPanel/Production) ---
# Panggil fungsi untuk memuat model dan aset saat aplikasi dimulai
# Ini akan dijalankan saat aplikasi di-import oleh WSGI server
load_model_assets()
...
if __name__ == '__main__':
    if model_loaded_successfully:
        ...
        # Jalankan server Flask untuk development
        app.run(host='0.0.0.0', port=5002, debug=False)  # Ubah debug=False untuk production
    else:
