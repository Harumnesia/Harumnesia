...
const FeatureCard = ({ title, description, icon }) => {
  return (
    <div className="group relative bg-gradient-to-br from-white via-white to-cream rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 flex flex-col items-center shadow-lg border border-gold/10 hover:border-gold/30 transform-gpu hover-lift-card backdrop-blur-sm">
      {/* Optimized icon container */}
      <div className="relative bg-gradient-to-br from-gold via-gold to-gold/80 w-16 h-16 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl flex items-center justify-center mb-6 sm:mb-8 shadow-lg shadow-smooth group-hover:shadow-xl">
        <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent rounded-xl sm:rounded-2xl"></div>
        <div className="relative z-10 scale-75 sm:scale-100 transition-transform duration-200 group-hover:scale-105">
          {icon}
        </div>
      </div>

      <h3 className="relative z-10 font-playfair font-bold text-xl sm:text-2xl md:text-3xl text-dark-gray mb-4 sm:mb-6 text-center leading-tight smooth-colors group-hover:text-dark-blue">
        {title}
      </h3>

      <p className="relative z-10 font-inter font-medium text-light-gray text-center text-sm sm:text-base md:text-lg leading-relaxed smooth-colors group-hover:text-dark-gray max-w-sm">
        {description}
      </p>
    </div>
  );
